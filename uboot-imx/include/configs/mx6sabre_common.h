...
#define CONFIG_BOOTANDROID \
	"run mmcargs;"	\
	"fatload mmc 2:1 ${loadaddr} boot.img;"	\
	"boota ${loadaddr}"
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"bootcmd_mmc_linux="CONFIG_BOOTLINUX "\0"	\
	"bootcmd_nfs="CONFIG_BOOT_NFS "\0"	\
	"bootcmd_mmc_android="CONFIG_BOOTANDROID "\0"	\
	"fdt_addr=0x18000000\0" \
	"boot_fdt=try\0" \
	"ip_dyn=yes\0" \
	...
	"bootcmd_android_recovery=run mmcargs;boota mmc2 recovery\0" \
	"update_sd_firmware=" \
		"if test ${ip_dyn} = yes; then " \
			"setenv get_cmd dhcp; " \
		"else " \
	...
	"netargs=setenv bootargs console=${console},${baudrate} ${smp} " \
		...
		"${mxcfb0} ${mxcfb1} ${mxcfb2} ${mxcfb3} vmalloc=400M androidboot.console=ttySAC0 androidboot.hardware=freescale\0"	\
